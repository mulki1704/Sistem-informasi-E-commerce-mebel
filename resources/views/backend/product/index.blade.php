                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($products as $data)
                                                <tr>
                                                    ...
                                                    <td data-title="Harga">Rp. {{ number_format($data->harga, 0, ',', '.') }}
                                                    </td>
                                                    @if ($data->status == 'Tersedia')
                                                        <td data-title="Status" class="text-success fw-bold">
                                                            {{ $data->status }}</td>
                                                    @else
                                                        <td data-title="Status" class="text-danger fw-bold">
                                                            {{ $data->status }}</td>
                                                    @endif
                                                    <td data-title="Deskripsi">{{ $data->deskripsi }}</td>
                                                            <td data-title="Image"><img src="{{ asset(ltrim($data->image_url, '/')) }}"
                                                                alt="{{ $data->nama }}" class="w-130 img-fluid" height="130px">
                                                    </td>
                                                        ...
                                                        <form action="{{ route('product.destroy', $data->id) }}"
                                                            ...
                                                            <a href="{{ route('product.show', $data->id) }}"
                                                                class="btn btn-sm btn-outline-warning"><i
                                                                    class="fa fa-info mr-1"></i>
                                                                Show
                                                            </a> |
                                                            <button type="submit" class="btn btn-sm btn-outline-danger"
                                                                onclick="return confirm('Apakah Anda Yakin?')"><i
                                                                    class="fa fa-trash-o mr-1"></i>
                                                                Delete
                                                            </button>
                                                        </form>
                                                    </td>
